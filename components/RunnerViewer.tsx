...
const RunnerViewer: React.FC<RunnerViewerProps> = ({ projectId, project: initialProject, onClose, onToggleImmersive }) => {
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 z-50 bg-cassette-dark flex flex-col animate-fade-in font-mono text-gray-200">
        ...
        {isImmersiveMode && (
            <div 
                className="absolute top-0 left-0 right-0 h-8 z-[60] flex justify-center items-start pt-2 cursor-pointer group transition-all active:opacity-100"
                onClick={handleExitImmersive}
                onTouchEnd={handleExitImmersive}
            >
                <div className="w-20 h-1.5 bg-white/20 rounded-full shadow-lg backdrop-blur-md group-hover:bg-cassette-accent/80 transition-colors border border-white/5"></div>
            </div>
        )}
            ...
            <div className="absolute inset-0 bg-black/90 backdrop-blur-sm z-30 flex items-center justify-center p-6 animate-fade-in">
                <div className="bg-cassette-plastic w-full max-w-sm p-6 rounded-sm shadow-2xl border border-cassette-accent">
                    ...
                    <div className="space-y-5">
                        ...
                        <button onClick={() => setIsImmersiveMode(!isImmersiveMode)} className={`w-full p-4 border rounded-sm font-bold text-xs uppercase flex justify-between items-center ${isImmersiveMode ? 'bg-cassette-highlight text-black border-transparent' : 'border-white/20 text-gray-400 hover:text-white'}`}>
